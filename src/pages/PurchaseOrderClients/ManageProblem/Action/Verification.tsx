...
export default function VerificationProductManageFixingOrderClient() {
  const navigate = useNavigate();
  const location = useLocation();
  const { toast } = useToast();
  const { booster, accountsBooster } = useAuth();
  ...
  useEffect(() => {
    if (purchaseOrderClientId) {
      loadAllProducts();
      if (purchaseOrderClientProblemId) {
        handleDetailProduct();
        validationStock();
        accountsBooster({ responsibility_code: "DRIVER", search: "" });
      }
    }
  }, [purchaseOrderClientId, purchaseOrderClientProblemId]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const apiData = {
        ...
        pick_up_by: detailProblem?.problem_product_take_by,
        ...
        items: formData.items.map((item) => ({
          ...
          quantity_distribution: Number(item.quantity_distribution),
          reason: item.reason,
      ...
      if (response.status) {
        ...
        navigate("/incoming/fixing-order-clients/detail", {
      ...
      if (response.status && response.data) {
        ...
        const items =
          data.items?.map((item: any, index: number) => ({
            ...
            reason_warehouse: item.reason_warehouse,
            quantity_distribution: item.quantity_distribution,
            reason: item.reason || "",
          })) || [];
        ...
        setFormData({
          purchase_order_client_id:
            data.purchase_order_client_id || purchaseOrderClientId,
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center justify-between">
            <span className="text-lg font-semibold">Formulir Pengembalian</span>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                <Select
                  value={detailProblem?.problem_product_take_by}
                  onValueChange={(value) =>
                    setDetailProblem({
                      ...detailProblem,
                      problem_product_take_by: value,
                ...
                >
                  <SelectTrigger>
                    <SelectValue placeholder="Belum Diambil Dari klient" />
                  </SelectTrigger>
                  <SelectContent>
                    {booster?.map((driver) => (
                    ...
                    <Card key={item.id} className="border">
                      <CardContent className="p-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          ...
                          {/* Quantity Stock Opname*/}
                          <div className="space-y-2">
                            <Label>Masukan Ke Stok Opname</Label>
                            <Input
                              type="number"
                              max={item?.adjustment_quantity}
                              min="0"
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="flex gap-2">
                            <Button
                              ...
                              size="sm"
                              onClick={() => {
                                updateItem(item.id, "reason_warehouse", null),
                                  updateItem(
                                    item.id,
                                    "updated_at_warehouse",
                                    dateTimeNow()
                            ...
                            <Button
                              ...
                              onClick={() => {
                                updateItem(
                                  ...
                                  updateItem(item.id, "reason_warehouse", "");
                              }}
                            >
