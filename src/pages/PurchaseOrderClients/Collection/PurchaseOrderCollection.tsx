...
export default function PurchaseOrderClientCollection() {
  ...
  const fetchPurchaseOrderClient = async (params) => {
    try {
      const { data } = await API.get(
        `/purchase-order/clients/sales?page=${params.page}&size=${params.size}&progress_type_code=${params.progress_type_code}&status_trx_code=PENDING&search=${params.search}&payment_method_code=${params.payment_method_code}&start_date=${params.start_date}&end_date=${params.end_date}`
      );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 className="text-3xl font-bold text-foreground">Tagihan Klien</h1>
          <p className="text-muted-foreground">
            Kelola dan tinjau tagihan pesanan klien
          </p>
        </div>
      </div>
                    ...
                    <TableRow key={user.id}>
                      ...
                      <TableCell>
                        <div className="flex items-center gap-2">
                          <Button
                            onClick={() =>
                              navigate(`/po-client-collection/detail`, {
                                state: user?.id,
                              })
                            }
                            variant="ghost"
