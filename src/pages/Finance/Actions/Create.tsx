"use client";

import React, { useEffect, useState } from "react";
import { Button } from "@/components/ui/button";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import { Textarea } from "@/components/ui/textarea";
import {
  Select,
  SelectTrigger,
  SelectContent,
  SelectItem,
  SelectValue,
} from "@/components/ui/select";
import { useToast } from "@/hooks/use-toast";
import { useNavigate, useLocation } from "react-router-dom";
import {
  useFinance,
  FormStateFinanceInOut,
  FormStateCategoryCode,
  FormStateFinanceEdit,
} from "@/contexts/Finance.context";
import PriceInput from "@/components/ui/PriceInput";
import { Trash2, Upload } from "lucide-react";
import {
  AlertDialog,
  AlertDialogTrigger,
  AlertDialogContent,
  AlertDialogHeader,
  AlertDialogTitle,
  AlertDialogDescription,
  AlertDialogFooter,
  AlertDialogCancel,
  AlertDialogAction,
} from "@/components/ui/alert-dialog";
import { CategoryModal } from "./CategoryModal";
import { ConfirmModal } from "@/components/ui/ConfimModal";
import { formatDate } from "@/components/format/Date";
import { usePurchaseOrderClients } from "@/contexts/PurchaseOrderClient.Context";

export default function CreateFinanceInOutPage() {
  const navigate = useNavigate();
  const location = useLocation();
  const { toast } = useToast();

  const type = location.state?.type || "INCOME";
  const statusCode = location.state?.statusCode || [];
  const refund = location.state?.refund || null;
  const {
    createFinance,
    getCategoryCode,
    categoryCode,
    addCategoryCode,
    updateCategoryCode,
    deleteCategoryCode,
  } = useFinance();

  const [form, setForm] = useState<FormStateFinanceInOut>({
    finance_code: type,
    input_date: "",
    description: "",
    category_code: "",
    total: 0,
    status_code: "",
    source_id: null,
    source_table: null,
    path: null,
  });

  const [categoryForms, setCategoryForms] = useState<FormStateCategoryCode>({
    lookup_code: type === "INCOME" ? "CATEGORY_INCOME" : "CATEGORY_EXPENSE",
    lookup_value_code: "",
    value: "",
    description: "",
  });
  const [categoryId, setCategoryId] = useState<number | null>(null);
  const [preview, setPreview] = useState<string | null>(null);

  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;

    const reader = new FileReader();
    reader.onloadend = () => {
      const base64 = reader.result as string;
      setForm((prev) => ({ ...prev, path: base64 }));
      setPreview(base64);
    };
    reader.readAsDataURL(file);
  };

  const handleSubmit = async () => {
    if (
      !form.input_date ||
      !form.description ||
      !form.category_code ||
      !form.status_code
    ) {
      toast({
        title: "Gagal",
        description: "Semua field wajib diisi (kecuali gambar).",
        variant: "destructive",
      });
      return;
    }

    try {
      const response = await createFinance(form);
      if (response.status) {
        toast({
          title: "Berhasil",
          description: `${
            type === "INCOME" ? "Pemasukan" : "Pengeluaran"
          } berhasil dibuat.`,
        });
        navigate(`${type === "INCOME" ? "/finance-in" : "/finance-out"}`);
      } else {
        toast({
          title: "Gagal",
          description: `${
            type === "INCOME" ? "Pemasukan" : "Pengeluaran"
          } gagal dibuat.`,
        });
      }
    } catch (error) {
      toast({
        title: "Error",
        description: "Terjadi kesalahan saat membuat data.",
        variant: "destructive",
      });
    }
  };

  // category create and update
  useEffect(() => {
    if (refund) {
      setForm({
        ...form,
        total: refund.total_refund,
        source_id: refund?.id,
        source_table: "purchase_order_problem_clients",
        status_code: "PENDING",
        category_code: "REFUND",
        input_date: formatDate(new Date(), "YYYY-MM-DD"),
        description: `Uang Refund Purchase Order Klien kepada ${refund?.client_name}`,
      });
    }
    getCategoryCode({
      size: 1000,
      page: 1,
      lookup_code: `${
        type === "INCOME" ? "CATEGORY_INCOME" : "CATEGORY_EXPENSE"
      }`,
    });
  }, []);

  const handleCategorySubmit = async () => {
    try {
      const newForm = {
        ...categoryForms,
        lookup_value_code: categoryForms.value
          .toUpperCase()
          .replace(/\s+/g, "_"),
      };
      const res = await addCategoryCode(newForm);
      if (res.status) {
        toast({
          title: "Berhasil",
          description: `Kategori baru berhasil ditambahkan.`,
        });
        getCategoryCode({
          size: 100,
          page: 1,
          lookup_code: `${
            type === "INCOME" ? "CATEGORY_INCOME" : "CATEGORY_EXPENSE"
          }`,
        });
      } else {
        toast({
          title: "Gagal",
          description:
            res?.messages || "Gagal menambahkan kategori. Silakan coba lagi.",
          variant: "destructive",
        });
      }
    } catch (error) {
      console.error("Error adding category:", error);
      toast({
        title: "Error",
        description: "Terjadi kesalahan saat menambahkan kategori.",
        variant: "destructive",
      });
    }
  };

  // update
  const handleEditCategory = async (item: any) => {
    try {
      const updatedForm = {
        lookup_value_code: categoryForms?.value
          .toUpperCase()
          .replace(/\s+/g, "_"),
        lookup_value_id: item.lookup_value_id,
        value: categoryForms?.value,
        description: categoryForms.description,
        lookup_code: type === "INCOME" ? "CATEGORY_INCOME" : "CATEGORY_EXPENSE",
      };
      const res = await updateCategoryCode(updatedForm);
      if (res.status) {
        toast({
          title: "Berhasil",
          description: `Kategori berhasil diperbarui.`,
          variant: "default",
        });
        getCategoryCode({
          size: 100,
          page: 1,
          lookup_code: `${
            type === "INCOME" ? "CATEGORY_INCOME" : "CATEGORY_EXPENSE"
          }`,
        });
      } else {
        toast({
          title: "Gagal",
          description:
            res?.messages || "Gagal memperbarui kategori. Silakan coba lagi.",
          variant: "destructive",
        });
      }
    } catch (error) {
      console.log("Error edit category:", error);
      toast({
        title: "Error",
        description: "Terjadi kesalahan saat mengedit kategori.",
        variant: "destructive",
      });
    }
  };

  // delete
  const handleDeleteCategory = async (id) => {
    try {
      const res = await deleteCategoryCode(id);
      if (res.status) {
        toast({
          title: "Berhasil",
          description: `Kategori berhasil dihapus.`,
          variant: "default",
        });
        getCategoryCode({
          size: 100,
          page: 1,
          lookup_code: `${
            type === "INCOME" ? "CATEGORY_INCOME" : "CATEGORY_EXPENSE"
          }`,
        });
      } else {
        toast({
          title: "Gagal",
          description:
            res?.messages || "Gagal menghapus kategori. Silakan coba lagi.",
          variant: "destructive",
        });
      }
    } catch (error) {
      toast({
        title: "Error",
        description: " Terjadi kesalahan saat menghapus kategori.",
        variant: "destructive",
      });
    }
  };
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-6 md:flex-row md:items-start md:justify-between">
        <div className="w-full md:w-4/5">
          <h1 className="text-3xl font-bold text-foreground">
            Tambah {type === "INCOME" ? "Pemasukan" : "Pengeluaran"}
          </h1>
          <p className="text-muted-foreground">
            Buat {type === "INCOME" ? "pemasukan" : "pengeluaran"} perusahaan.
          </p>
        </div>
        <div className="flex flex-row gap-x-3">
          <CategoryModal
            onSubmit={handleCategorySubmit}
            type={type}
            categoryForms={categoryForms}
            setCategoryForms={setCategoryForms}
            children={<Button variant="outline">Tambah Kategori</Button>}
          />

          <Button onClick={handleSubmit}>
            {type === "INCOME" ? "Simpan Pemasukan" : "Simpan Pengeluaran"}
          </Button>
          {refund && (
            <Button
              onClick={() =>
                navigate("/review-order-client", {
                  state: {
                    purchase_order_client_id: refund?.purchase_order_client_id,
                    page: "ADJUSTMENT",
                  },
                })
              }
            >
              Lihat Detail Pesanan
            </Button>
          )}
        </div>
      </div>

      <div className="grid gap-6 md:grid-cols-3">
        <Card className="md:col-span-3">
          <CardHeader>
            <CardTitle>
              Informasi {type === "INCOME" ? "Pemasukan" : "Pengeluaran"}
            </CardTitle>
          </CardHeader>

          <CardContent className="space-y-6">
            <div className="grid gap-4 md:grid-cols-2">
              {/* Tanggal Input */}
              <div className="space-y-2">
                <Label>Tanggal Input</Label>
                <Input
                  type="date"
                  value={form.input_date}
                  onChange={(e) =>
                    setForm({ ...form, input_date: e.target.value })
                  }
                />
              </div>
              {/* Status */}
              <div className="space-y-2">
                <Label>Status</Label>
                <Select
                  value={form.status_code}
                  onValueChange={(val) =>
                    setForm({ ...form, status_code: val })
                  }
                >
                  <SelectTrigger>
                    <SelectValue placeholder="Pilih status" />
                  </SelectTrigger>
                  <SelectContent>
                    {statusCode.map((item) => (
                      <SelectItem
                        key={item.lookup_value_code}
                        value={item.lookup_value_code}
                      >
                        {item.value}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              </div>
            </div>

            {/* Deskripsi */}
            <div className="space-y-2">
              <Label>Deskripsi</Label>
              <Textarea
                placeholder="Contoh: Beli Bitcoin"
                value={form.description}
                onChange={(e) =>
                  setForm({ ...form, description: e.target.value })
                }
              />
            </div>

            <div className="grid gap-4 md:grid-cols-2">
              {/* Kategori */}
              <div className="space-y-2">
                <Label>Kategori</Label>
                <Select
                  value={form.category_code}
                  onValueChange={(val) =>
                    setForm({ ...form, category_code: val })
                  }
                >
                  <SelectTrigger>
                    <SelectValue placeholder="Pilih kategori" />
                  </SelectTrigger>
                  <SelectContent>
                    {categoryCode.map((item, index) => (
                      <div
                        key={item.lookup_value_code}
                        className="flex items-center justify-between px-2"
                      >
                        <SelectItem
                          value={item.lookup_value_code}
                          className="flex-1"
                        >
                          {item.value}
                        </SelectItem>
                        <div className="flex flex-row gap-2 items-center">
                          <CategoryModal
                            categoryCodeData={item}
                            onSubmit={handleEditCategory}
                            categoryForms={categoryForms}
                            setCategoryForms={setCategoryForms}
                            type={type}
                            isEdit={true}
                            children={
                              <Button
                                variant="ghost"
                                size="icon"
                                className="p-0"
                              >
                                <Upload className="w-4 h-4" />
                              </Button>
                            }
                          />
                          <ConfirmModal
                            title="Hapus Pesanan"
                            description={`Apakah kamu yakin ingin menghapus pemasukan${" "}<b>${
                              item.value
                            }</b>? Tindakan ini tidak dapat dibatalkan.`}
                            confirmText="Iya"
                            cancelText="Batal"
                            variant="outline"
                            showIcon={false}
                            useHTML
                            trigger={
                              <Button
                                variant="ghost"
                                size="icon"
                                className="h-8 w-8 text-destructive"
                              >
                                <Trash2 />
                              </Button>
                            }
                            onConfirm={() =>
                              handleDeleteCategory(item.lookup_value_id)
                            }
                          />
                        </div>
                      </div>
                    ))}
                  </SelectContent>
                </Select>
              </div>

              {/* Total */}
              <div className="space-y-2">
                <Label>Total</Label>
                <PriceInput
                  value={form.total}
                  onChange={(value) =>
                    setForm({ ...form, total: Number(value) })
                  }
                />
              </div>
            </div>

            {/* Upload Gambar */}
            <div className="space-y-2">
              <Label>Bukti Transaksi (Opsional)</Label>
              <div className="flex items-center gap-4">
                {preview && (
                  <div className="w-20 h-20 border rounded-md overflow-hidden">
                    <img
                      src={preview}
                      alt="preview"
                      className="w-full h-full object-cover"
                    />
                  </div>
                )}
                <div>
                  <Input
                    type="file"
                    accept="image/*"
                    onChange={handleImageChange}
                    className="hidden"
                    id="driver-photo"
                  />
                  <div className="flex flex-col gap-2">
                    <Label htmlFor="driver-photo" className="cursor-pointer">
                      <Button variant="outline" type="button" asChild>
                        <span>
                          <Upload className="w-4 h-4 mr-2" />
                          Unggah Foto
                        </span>
                      </Button>
                    </Label>
                  </div>
                </div>
              </div>
            </div>

            {/* Tombol Simpan */}
          </CardContent>
        </Card>
      </div>
    </div>
  );
}
