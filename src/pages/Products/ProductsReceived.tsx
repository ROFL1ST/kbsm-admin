  ...
  const fetchPurchaseOrderClient = async (params) => {
    try {
      const { data } = await API.get(
        `/purchase-order/vendors?page=${params.page}&size=${params.size}&progress_type_code=ORDER_RECEIVED_WAREHOUSE&status_trx_code=${params.status_trx_code}&search=${params.search}&payment_method_code=${params.payment_method_code}&start_date=${params.start_date}&end_date=${params.end_date}`
      );
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 className="text-3xl font-bold text-foreground">
            Pesanan Yang Sudah Datang
          </h1>
          <p className="text-muted-foreground">
            pesanan yang sudah masuk stok gudang
          </p>
        </div>
      </div>
