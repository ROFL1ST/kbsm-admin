...
export default function ProductsPreparationPacking() {
  ...
  const fetchPurchaseOrderClient = async (params) => {
    try {
      const { data } = await API.get(
        `/purchase-order/clients?page=${params.page}&size=${params.size}&progress_type_code=FOLLOW_UP&status_trx_code=${params.status_trx_code}&search=${params.search}&payment_method_code=${params.payment_method_code}&start_date=${params.start_date}&end_date=${params.end_date}`,
      );
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 className="text-3xl font-bold text-foreground">
            Pesanan Yang Sudah Tervalidasi
          </h1>
          <p className="text-muted-foreground">
            Pesanan yang sudah tervalidasi dan siap untuk diproses lebih lanjut,
            seperti packing dan pengiriman. Pastikan untuk memeriksa detail
            pesanan sebelum melanjutkan ke tahap berikutnya.
          </p>
        </div>
      </div>
      ...
      {dataPOClients?.length > 0 ? (
        ...
        <div className="text-center py-12">
          ...
          <p className="text-gray-400 mb-4">
            Tidak ditemukan data Pesanan yang sudah Selesai
          </p>
        </div>
      )}
    </div>
