...
export default function ProductsReceivedDetail() {
  ...
  const handleCreatePurchaseOrder = async () => {
    try {
      ...
      const response = await createOnePurchaseOrderVendorDeliveryReceived(
        updateData
      );
      if (response.status)
        toast({
  ...
  useEffect(() => {
    if (detailPurchaseOrderVendorDelivery) {
      ...
      const inputDate = new Date(data?.input_date).toISOString().split("T")[0];
      const sendDate = new Date(data?.send_date).toISOString().split("T")[0];
  ...
  const handleCancel = () => {
    setForm(undefined); // kosongkan seluruh form
    setDriverInfo({
      driver_name: "",
      driver_phone: "",
      driver_photo: null,
  ...
  const setMaxReceivedQuantity = (index: number) => {
    ...
    updateItem(index, "received_quantity", maxQuantity);
  };
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Data Histori Penerimaaan Barang
          </p>
        </div>
                ...
                <div className="space-y-4">
                  ...
                  {form?.products?.map((item, idx) => (
                    ...
                    >
                      ...
                      <div className="space-y-2">
                        <div className="flex justify-between items-center">
                          <Label>Total Produk yang Datang</Label>
