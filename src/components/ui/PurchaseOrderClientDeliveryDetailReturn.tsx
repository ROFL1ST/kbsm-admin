...
import { Camera, PencilOff } from "lucide-react";
import { set } from "date-fns";
import { useNavigate } from "react-router-dom";
import PurchaseOrderClientsAdjusments from "./PurchaseOrderClientsAdjusments";
import { FormAdjustmentProducts } from "./FormAdjusmentProducts";
...
export default function PurchaseOrderClientDeliveryDetailReturn({
  ...
  const validation = () => {
    ...
    form.items.forEach((item, index) => {
      ...
      const hasAdjustmentType = item.adjustment_type !== null;
      const hasReason = item.reason != null;
      const hasQuantity =
        item.adjustment_quantity && Number(item.adjustment_quantity) > 0;

      // Aturan: Jika salah satu field diisi, maka ketiganya harus diisi
      if (hasAdjustmentType || hasQuantity || hasReason) {
        const errors = [];

        if (!hasReason) {
          errors.push("Alasan Pengembalian produk");
        }
        if (!hasQuantity) {
          errors.push("Quantity Pengembalian");
        }
        if (!hasAdjustmentType) {
          errors.push("Tipe Pengembalian");
        }

        // Jika ada error di item ini
        if (errors.length > 0) {
          toast({
            title: "Gagal",
            description: `Tolong Masukan ${errors.join(
              ", "
            )}, pada baris ke-${itemIndex}`,
          });
          hasError = true;
        }
      }
  ...
  const handleSubmitAdjusmentProducts = async (clear: boolean) => {
    setIsLoading(true);
    if (clear) {
      // Create updated form data for clearing
      const updatedForm = {
        ...form,
        is_problem_products: "N",
        problem_product_take_by: null,
      ...
      // Update local state
      setForm(updatedForm);
      setIsAdjusmentProduct(false);
      // Send to API
      try {
        const response = await updatePurchaseOrderClient(updatedForm);
        if (response.status) {
          toast({
            title: "Berhasil",
        ...
        console.error("Error clearing adjustment products:", error);
      ...
      // Submit regular adjustment
      try {
        const response = await updatePurchaseOrderClient(form);
        if (response.status) {
          toast({ title: "Berhasil", description: "Pengajuan Sudah Terkirim" });
        } else {
          toast({ title: "Gagal", description: "Sedang Maintenance" });
        }
      } catch (error) {
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Lihat Detail Pengriman</p>
        </div>
        {/* button */}
      </div>
      <LocationInformation
        ...
        send_date={
          detailPurchaseOrderClient?.return
            ? detailPurchaseOrderClient?.return?.send_to_client_date
            : detailPurchaseOrderClient?.send_date
        }
        progress_type_code={
          detailPurchaseOrderClient?.return
            ? detailPurchaseOrderClient?.return?.progress_delivery_return
            : detailPurchaseOrderClient?.progress_type_code
        }
