...
import {
  PurchaseOrderVendorCollectionFormState,
  usePurchaseOrderVendors,
} from "@/contexts/PurchaseOrderVendors.Context";
import PriceInput from "./PriceInput";
export default function HistoryCollectionVendor({
  ...
  const [form, setForm] = useState<PurchaseOrderVendorCollectionFormState>({
    purchase_order_vendor_id: purchase_order_vendor_id,
    purchase_order_collection_vendor_id: undefined,
  ...
  const handleInputChange = (
    field: keyof PurchaseOrderVendorCollectionFormState,
    value: any
  ) => {
    setForm((prev) => ({
      ...prev,
  ...
  const handleEditHistory = (item, index) => {
    setForm({
      purchase_order_vendor_id: purchase_order_vendor_id,
      purchase_order_collection_vendor_id: item.id,
  ...
  const handleDeleteHistory = async (id) => {
    const response = await deletePurchaseOrderCollections({
      purchase_order_collection_vendor_id: id,
    });
    if (response.status) {
      toast({
        title: "Berhasil",
        ...
        <CardContent className="p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-8">
              ...
              <div className="">
                ...
                <p className="text-lg font-bold text-amber-600">
                  {formatIDR(
                    invoiceInfo.total - invoiceInfo.paid > 0
          ...
          <div className="mt-4">
            <div className="flex justify-between text-sm text-muted-foreground mb-1">
              ...
              <span>
                {Math.round((invoiceInfo.paid / invoiceInfo.total) * 100)}%
              </span>
            </div>
            <div className="w-full bg-gray-200 rounded-full h-2">
              <div
                ...
                style={{
                  width: `${
                    (invoiceInfo.paid / invoiceInfo.total) * 100 >= 100
                      ? 100
                      : (invoiceInfo.paid / invoiceInfo.total) * 100
                  }%`,
                }}
              ></div>
            </div>
      ...
      {!showHistoryDetail && (
        <section className="mt-6 bg-white dark:bg-zinc-900 border border-primary/30 rounded-2xl shadow-sm p-5">
          ...
          <p className="text-sm text-muted-foreground mb-5">
            Daftar histori tagihan pesanan Vendor
          </p>
                        ...
                        <ConfirmModal
                          ...
                          trigger={
                            <Button
                              variant="outline"
                              size="sm"
                              disabled={item?.type === 2 ? true : false}
            ...
            <CardContent className="space-y-6">
              ...
              <div className="flex justify-end gap-2 pt-4">
                ...
                <Button
                  ...
                  disabled={!form.price || !form.path || data?.type === 2}
                >
