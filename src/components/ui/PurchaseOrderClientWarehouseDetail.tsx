// app/purchase-order-client/[id]/page.tsx atau PurchaseOrderClientWarehouseDetail.tsx
"use client";
...
import ShippingInfoCard, {
  ShippingData,
} from "@/components/ui/ShippingInfoCard";
...
export default function PurchaseOrderClientWarehouseDetail({
  ...
  // Handle tracking package
  const handleTrackPackage = (resi: string, courierCode: string) => {
    // Bisa disesuaikan dengan API tracking yang digunakan
    const trackingUrls: Record<string, string> = {
      jne: `https://tracking.jne.co.id/Detail/${resi}`,
      pos: `https://tracking.posindonesia.co.id/tracking/${resi}`,
      tiki: `https://www.tiki.id/tracking/${resi}`,
      sicepat: `https://sicepat.com/tracking/${resi}`,
      anteraja: `https://anteraja.id/tracking/${resi}`,
    };

    const url =
      trackingUrls[courierCode?.toLowerCase()] ||
      `https://cekresi.com/${courierCode}/${resi}`;
    window.open(url, "_blank");
  };

  // Handle copy resi
  const handleCopyResi = (resi: string) => {
    // Tambahkan toast notification di sini
    console.log("Resi copied:", resi);
    // Contoh dengan toast sederhana
    alert(`Nomor resi ${resi} telah disalin`);
  };
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white dark:from-gray-900 dark:to-gray-950">
      ...
      <div className="container mx-auto px-4 py-4 lg:py-8 max-w-7xl">
        ...
        <div className="grid gap-4 lg:gap-6">
          ...
          {/* Shipping Info Card - INTEGRATED HERE */}
          <ShippingInfoCard
            shipping={detailPurchaseOrderClient?.shipping}
